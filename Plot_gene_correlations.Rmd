...
# --- Load data
```{r}
# Load and assign rownames to column
path_list <- list(
  Covar_adjusted = read.csv("../data/neck_fat_rnaseq_adj_covars/gene_stats.csv", row.names = 1)
)

path_list[[1]] <- path_list[[1]] %>% rownames_to_column("ensembl_id")
```

# --- Make long dataset and format values
```{r}
...
```{r}
# Select the vars of interest and create full name column
...
final_df <- final_df %>%
  ...
  filter(!is.na(var_labels)) # remove variables that are not expliclty named
```
...
plot_data %>% filter(symbol %in% c("ADRB3", "PRDM16", "UCP1", "PPARGC1A"), var == "FreeT4")
# Save plot
scale <- 95
...
write.csv(
  plot_data %>%
    select(-c(var_labels, significance, stroke_width, significance_color, rounded_slope)),
  "../output/4 - Figures and Analysis/filtered_gene_correlations.csv",
  row.names = FALSE
)
write.csv(
  final_df %>%
    select(-var_labels),
  "../output/4 - Figures and Analysis/gene_correlations.csv",
  row.names = FALSE
)
```
