...
f.df = df %>% filter(!is.na(Sample)) %>% column_to_rownames("Sample")
...
meta = meta %>% mutate(Rows = paste0(tissue, subject)) %>% mutate(Rows = gsub("SubQ", "SC", Rows)) %>% column_to_rownames("Rows")
...
PlotGeneCorrelation <- function(genes_of_interest = "UCP1", FDR_threshold = 0.15){
  ...
  for (n in 1:length(ens_genes_of_interest)) {
    ...
    for (variable in names(meta)[c(3,4,7:9)]) {
      ...
      #Rename SC 
      plot_data = plot_data %>% rename("Tissue" = "tissue") %>% mutate(Tissue = case_when(Tissue == "SubQ" ~ "SC",
                                                                                          TRUE ~ Tissue))
      ...
      for(stat in names(deep_stat_col)){
        deep_stat[[stat]] = gene_stats %>% filter(X == ens_gene) %>% select(all_of(deep_stat_col[[stat]])) %>% pull(!!sym(deep_stat_col[[stat]]))
        sc_stat[[stat]] = gene_stats %>% filter(X == ens_gene) %>% select(all_of(sc_stat_col[[stat]])) %>% pull(!!sym(sc_stat_col[[stat]]))
      }
      ...
      deep_annot = paste0(sprintf("Slope: %.2f", deep_stat[["slope"]]), 
                          sprintf("; p-val: %.3f", deep_stat[["pval"]]),
                          sprintf("; FDR: %.2f", deep_stat[["FDR"]])
      )
      
      sc_annot = paste0(sprintf("Slope: %.2f", sc_stat[["slope"]]), 
